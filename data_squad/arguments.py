...
def parse():
    ...
    if args.world_size > 1:
        ...
        return
    args.timestamp = '-'.join(
        datetime.datetime.now(tz=tz.tzoffset(None, -8 * 60 * 60)).strftime(
            "%y/%m/%d/%H/%M/%S.%f").split())
    ...
    if len(args.train_tasks) > 1:
        ...
        if len(args.train_iterations) < len(args.train_tasks):
            args.train_iterations = len(
                args.train_tasks) * args.train_iterations
        if len(args.train_batch_tokens) < len(args.train_tasks):
            args.train_batch_tokens = len(
                args.train_tasks) * args.train_batch_tokens
